                      ...
                      int width, int height)
{
    ...
    const double minLat = -75.0;   // Pol Sud
    const double maxLat = 90.0;    // Pol Nord  
    const double minLon = -164.0;  // 180° Vest
    const double maxLon = 180.0;   // 180° Est
...
MapWidget& MapWidget::add_location(double x,double y){
    QPainter painter(&pixmap);
    QColor color(255, 0, 0, 28);
    painter.setBrush(color);
    painter.setPen(Qt::NoPen);
    painter.drawEllipse(latLonToPoint(x,y,w,h), 27, 27);
    painter.end();
    label->setPixmap(pixmap); 
    return *this;
}